...
use super::{PolicyViolation, FingerprintedViolation, ViolationDisposition};
...
mod tests {
    ...
    fn test_entry(rule: &str, severity: Severity, from_file: &str, disposition: ViolationDisposition) -> FingerprintedViolation {
    ...
    fn human_format_includes_all_fields() {
        let entry = test_entry("boundary.never_imports", Severity::Error, "src/app/main.ts", ViolationDisposition::New);
    ...
    fn baseline_violations_have_space_prefix_in_diff() {
        let entry = test_entry("boundary.never_imports", Severity::Warning, "src/app/main.ts", ViolationDisposition::Baseline);
        let output = format_violation_diff(Path::new("."), &entry);
    ...
    fn stats_format_includes_all_counts() {
        let violations = vec![
            ...
            test_entry("rule2", Severity::Warning, "b.ts", ViolationDisposition::New),
            test_entry("rule3", Severity::Error, "c.ts", ViolationDisposition::Baseline),
        ];
